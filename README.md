...
- **Dashboard.** Desktop has a fixed 240px side nav. Smaller screens use a drawer opened from a menu button in the top bar. The overview shows one row on desktop, the balance above a row of stats on tablet, and a single column on mobile. Shipment cards use a 2-column grid on mobile, and the banner and chart scale with width.
